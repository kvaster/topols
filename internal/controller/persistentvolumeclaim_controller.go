...
func (r *PersistentVolumeClaimReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Skip if the PVC is not deleted or PVC does not have TopoLS's finalizer.
	...
	controllerutil.RemoveFinalizer(pvc, topols.PVCFinalizer)
...
needResizing:
	...
	for _, pod := range pods {
		...
		lastResizeFSRequestedAt, err := time.Parse(time.RFC3339Nano, pod.Annotations[topols.LastResizeFSRequestedAtKey])
		if err == nil && lastResizeFSRequestedAt.After(condTransitionAt) {
			continue
		}

		pod.Annotations[topols.LastResizeFSRequestedAtKey] = time.Now().Format(time.RFC3339Nano)
