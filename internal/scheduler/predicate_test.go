	...
	return corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				topols.CapacityKeyPrefix + deviceClass1: fmt.Sprintf("%d", cap1Gb<<30),
				topols.CapacityKeyPrefix + deviceClass2: fmt.Sprintf("%d", cap2Gb<<30),
				topols.CapacityKeyPrefix + deviceClass3: fmt.Sprintf("%d", cap3Gb<<30),
			},
		},
	}
}
				...
				Items: []corev1.Node{
					...
					},
					{
						ObjectMeta: metav1.ObjectMeta{
							Name: "10.1.1.4",
							Annotations: map[string]string{
								topols.CapacityKeyPrefix + deviceClass1: "foo",
							},
						},
					},
				},
	...
	testCases := []struct {
	...
	}{
		{
			input: &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						topols.CapacityKeyPrefix + deviceClass1: strconv.Itoa(5 << 30),
					},
				},
				Spec: corev1.PodSpec{
		...
		},
		{
			input: &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						topols.CapacityKeyPrefix + deviceClass1: strconv.Itoa(3 << 30),
					},
				},
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{
